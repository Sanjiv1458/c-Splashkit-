...
void add_to_start(linked_list &list, int value)
{
    ...
    if (not list_is_empty(list))
    {
        list.first->next = added_node; // point first node to new node
        list.first = added_node;
    }
    else
    {
...
{
    node temp = new node_data();
    temp->data = value;
    temp->next = selected_node;
    selected_node = temp;
}
...
int lenth(const linked_list &list)
{
    int count = 0;
...
int main()
{
    ...
    add_to_end(my_list, 50);
    print_all(my_list);

    // Add additional nodes to the start of the list...
    //TODO: Add code to add to start here
    write_line("TEST: Add to start");
    add_to_start(my_list, 55);
    print_all(my_list);
    ...
    //TODO: Add code to test length here
    write_line("TEST: Length");
    write("Lenth: ");
    write_line(to_string(lenth(my_list)));

    write_line("adding new element!");
    //Add a new
    add_to_start(my_list, 65);
    add_to_end(my_list, 100);

    //after updation new lenth
    write_line("TEST: after updation Length");
    write("Lenth: ");
    write_line(to_string(lenth(my_list)));
    ...
    // insert after 100...
    node select;
    select = find_first(my_list, 100);
    insert_after(select, 200);
